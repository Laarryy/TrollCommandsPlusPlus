...
public class SlowMineCommand extends PluginCommand {
	//vars
	IRegistry slowmineRegistry = (IRegistry) ServiceLocator.getService(PluginServiceType.SLOWMINE_REGISTRY);
	...
	protected void execute() {
		if (sender instanceof Player && !permissionsManager.playerHasPermission((Player) sender, PermissionsType.COMMAND_SLOWMINE)) {
		...
		if (args.length == 1) {
			slowmine(args[0], Bukkit.getPlayer(args[0]));
		} else {
	...
	private void slowmine(String name, Player player) {
		if (player == null) {
		...
		if (slowmineRegistry.contains(name.toLowerCase())) {
			sender.sendMessage(name + " is no longer a slow miner.");
			slowmineRegistry.setRegister(name.toLowerCase(), null);
		} else {
			sender.sendMessage(name + " is now a slow miner.");
			slowmineRegistry.setRegister(name.toLowerCase(), player);
		}
