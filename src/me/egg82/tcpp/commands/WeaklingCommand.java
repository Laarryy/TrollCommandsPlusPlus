...
public class WeaklingCommand extends PluginCommand {
	//vars
	...
	protected void execute() {
		if (sender instanceof Player && !permissionsManager.playerHasPermission((Player) sender, PermissionsType.COMMAND_WEAKLING)) {
		...
		if (args.length == 1) {
			weakling(args[0], Bukkit.getPlayer(args[0]));
		} else {
	...
	private void weakling(String name, Player player) {
		if (player == null) {
		...
		if (weaklingRegistry.contains(name.toLowerCase())) {
