...
public class HurtCommand extends PluginCommand {
	...
	protected void execute() {
		if (sender instanceof Player && !permissionsManager.playerHasPermission((Player) sender, PermissionsType.COMMAND_HURT)) {
		...
		if (args.length == 1) {
			hurt(args[0], Bukkit.getPlayer(args[0]));
		} else {
	...
	private void hurt(String name, Player player) {
		if (player == null) {
		...
		if (hurtRegistry.contains(name.toLowerCase())) {
			sender.sendMessage(name + " is no longer being hurt.");
			hurtRegistry.setRegister(name.toLowerCase(), null);
		} else {
			sender.sendMessage(name + " is now being hurt.");
			hurtRegistry.setRegister(name.toLowerCase(), player);
		}
